function exp03_preproc(subjects_override)
% EXP03_PREPROC Entrypoint for preprocessing experiment 03.
% V 1.0.2
%
% Call chain:
%   exp03_preproc.m -> preproc_default.m -> preproc_core.m
...
try
    ...
    error('exp03_preproc:EEGLABInitFail', 'EEGLAB init failed: %s', ME.message);
end
...
if ~exist(cfg_path, 'file')
    error('exp03_preproc:MissingJSON', 'Config JSON not found: %s', cfg_path);
end
