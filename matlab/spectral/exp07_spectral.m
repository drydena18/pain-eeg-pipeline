...
%
% Call chain:
%   exp07_spectral.m -> spectral_default.m -> spectral_core.m
%
% Usage:
%   exp07_spectral();           % uses cfg.exp.subjects (or participants.tsv fallback)
%   exp07_spectral([1 2 3 10]); % override subjects list
...
% If file lives at: <repo>/matlab/spectral/exp07_spectral.m
...
try
    ...
    error('exp07_spectral:EEGLABInitFail', 'EEGLAB init failed: %s', ME.message);
end
...
if ~exist(cfg_path, 'file')
    error('exp07_spectral:MissingJSON', 'Config JSON not found: %s', cfg_path);
end
