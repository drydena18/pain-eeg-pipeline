...
%   Row 3 – Phase (if available) + p5_flag + BI_pre autocorrelation:
%           [3,1] Phase slow-alpha: circulat histogram (rose) of GA phase
%           [3,2] p5_flag heatmap [chan x trial] (binary: flagged unstable power)
...
title(tl, sprintf('sub-%03d | Pre-Stimulus Alpha Interaction Metrics', subjid), 'Interpreter', 'none', 'FontSize', 14, 'FontWeight', 'bold');
...
title(ax, 'BI_{pre} & RL_{pre} over trials', 'Interpreter', 'tex');
xlabel(ax, 'Trial');
...
% ================================================================
heatmap_items = {
    4, 'bi_pre', 'BI_{pre} [chan x trial]';
    5, 'delta_erd', '\DeltaERD [chan x trial]';
    6, 'cog_pre', 'CoG_{pre} [chan x trial]';
};

for r = 1:size(heatmap_items, 1)
    title_idx = heatmap_items{r, 1};
    fn = heatmap_items{r, 2};
    ttl = heatmap_items{r, 3};

    ax = nexttile(tl, title_idx);
...
% ================================================================
% ROW 3 – Phase row / p5_flag / BI_pre autocorrelation
% ================================================================
...
ax = nexttile(tl, 8);
hasFlag = isfield(featChan, 'p5_flag') && ~isempty(featChan.p5_flag) && ...
    any(featChan.p5_flag(:) > 0, 'all')

if isfield(featChan, 'p5_flag') && ~isempty(featChan.p5_flag)
    F = featChan.p5_flag;
    imagesc(ax, F);
    axis(ax, 'tight');
    set(ax, 'YDir', 'normal');
...
if numel(b_ok) >= 6
    ...
    yline(ax, 0, 'k');
    % 95% confidence bounds under H0; white noise
    ci = 1.96 / sqrt(numel(b_ok));
