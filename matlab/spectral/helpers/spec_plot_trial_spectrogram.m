...
if min(tMs) < 0 && max(tMs) > 0
    xline(ax, 0, 'w-',  'LineWidth', 1.5, 'Alpha', 0.85);
end

% Alpha sub-band boundaries (dashed white lines)
% Total alpha band edges
yline(ax, alpha.alpha_hz(1), 'w--', 'LineWidth', 1.0, 'Alpha', 0.75);
yline(ax, alpha.alpha_hz(2), 'w--', 'LineWidth', 1.0, 'Alpha', 0.75);
...
if isfield(alpha, 'slow_hz') && isfield(alpha, 'fast_hz')
    ...
    yline(ax, boundary, 'w:', 'LineWidth', 1.0, 'Alpha', 0.75);
end
