function exp03_spectral(subjects_override)
% EXP03_SPECTRAL Entrypoint for spectral feature extraction (Experiment 3).
% V 1.0.1
%
% Call chain:
%   exp03_spectral.m -> spectral_default.m -> spectral_core.m
%
% Usage:
%   exp03_spectral();           % uses cfg.exp.subjects (or participants.tsv fallback)
%   exp03_spectral([1 2 3 10]); % override subjects list
...
% If file lives at: <repo>/matlab/spectral/exp03_spectral.m
...
try
    ...
    error('exp03_spectral:EEGLABInitFail', 'EEGLAB init failed: %s', ME.message);
end
...
if ~exist(cfg_path, 'file')
    error('exp03_spectral:MissingJSON', 'Config JSON not found: %s', cfg_path);
end
