...
1. STC time-windowed maps  (src_render_stc_timepoints)
   ...
   Each render is a 4-view layout: left lateral, right lateral, left medial,
   right medial.
...
renderer via the PYOPENGL_PLATFORM=osmesa environment variable.
...
─────────────
    mne >= 1.0
    pyvista
    pyvistaqt  (may be required on some platforms)
...
def _multi_view_screenshot(brain, views: list[tuple[str, str]]) -> np.ndarray:
    ...
    Args:
        ...
        views : List of (hemi, view) tuples, e.g.
...
def src_render_roi_scalar(
    ...
    belonging to that label are filled with the scalar value. Vertices not
    ...
    for metric in metric_cols:
        ...
        try:
            ...
            for roi, row in roi_lookup.items():
                ...
                # lh vertices occupy indices [0, n_verts_lh).
                # rh vertices occupy indices [n_verts_lh, n_verts_lh + n_verts_rh).
                #
                # Custom ROIs built by merging lh + rh labels via src_build_custom_rois
                # produce a mne.BiHemiLabel whose .hemi == "both". BiHemiLabel exposes
                # sub-labels via .lh and .rh attributes, each a standard Label.
                hemi = getattr(matched_label, "hemi", None)
                lh_verts = src[0]["vertno"]
                rh_verts = src[1]["vertno"]

                if hemi == "lh":
                    idx = np.intersect1d(matched_label.vertices, lh_verts,
                                         return_indices=True)[2]
                    vtx_data[idx] = float(val)
                elif hemi == "rh":
                    idx = np.intersect1d(matched_label.vertices, rh_verts,
                                         return_indices=True)[2]
                    vtx_data[n_verts_lh + idx] = float(val)
                elif hemi == "both":
                    # BiHemiLabel: paint each hemisphere sub-label independently
                    lh_idx = np.intersect1d(matched_label.lh.vertices, lh_verts,
                                             return_indices=True)[2]
                    rh_idx = np.intersect1d(matched_label.rh.vertices, rh_verts,
                                             return_indices=True)[2]
                    vtx_data[lh_idx] = float(val)
                    vtx_data[n_verts_lh + rh_idx] = float(val)
            ...
            # Symmetric colour scale centred at 0 for signed metrics;
