"""
exp03_source.py - Entry point for source localization (Experiment 3)
V 2.1.0

Call Chain:
    exp03_source.py -> source_default.py -> source_core.py
...
def exp02_source(subjects_override = None, cfg_path: str = None):
    ...
    Args:
        ...
        cfg_path          : Optional explicit path to the experiment JSON config.
                            Defaults to <repo_root>/config/exp03.json.
    """
    exp_id = "exp03"

    cfg_path = "/home/UWO/darsenea/Documents/GitHub/pain-alpha-dynamics/config/exp03.json"

    if cfg_path is None:
        repo_root = os.path.dirname(os.path.join(_this_dir, "..", ".."))
        cfg_path = os.path.join(repo_root, "config", f"{exp_id}.json")

    print(f"[exp03_source] Script dir: {_this_dir}")
    print(f"[exp03_source] Config path: {cfg_path}")
