"""
exp07_source.py - Entry point for source localization (Experiment 1)
V 2.0.0

Call Chain:
    exp07_source.py -> source_default.py -> source_core.py
...
Usage (command line):
    python exp07_source.py
    python exp07_source.py --subjects 1 2 3 10
    python exp07_source.py --cfg /path/to/exp07.json

Config JSON expected at: <repo_root>/config/exp07.json
...
def exp07_source(subjects_override = None, cfg_path: str = None):
    """
    ...
    Args:
        ...
        cfg_path          : Optional explicit path to the experiment JSON config.
                            Defaults to <repo_root>/config/exp07.json.
    """
    exp_id = "exp07"
...
def _parse_args():
    ...
    ap.add_argument("--cfg", default = None, metavar = "PATH",
                    help = "Path to experiment JSON config (default: <repo>/config/exp07.json)")
...
if __name__ == "__main__":
    args = _parse_args()
    exp07_source(subjects_override = args.subjects, cfg_path = args.cfg)